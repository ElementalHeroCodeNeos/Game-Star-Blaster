...
public class Commander extends Enemy {    
    public Commander(double vx, double vy, int width, int height, int health, int maxHealth, int power, int point, BufferedImage image, boolean isActive){
        super(vy, width, height, health, maxHealth, power, point, image, isActive);
        this.vx = vx;
    }
    
    public Commander(double vx, double vy, int width, int height, int health, int maxHealth, int power, int point, BufferedImage image, boolean isActive, int maxBullets, int powerBullet){
        super(vy, width, height, health, maxHealth, power, point, image, isActive);
        this.vx = vx;
        this.maxBullets = maxBullets;
        this.bullets = new Bullet[maxBullets];
        for(int i=0; i<this.maxBullets; i++){
            this.bullets[i] = new Bullet(2.5, 48, 48, powerBullet, false);
        }
    }

    @Override
    public void move(long timeCounter, double playerX){
        if(type == 0) super.parabolMove();
        else super.sideMove(WIDTH);
    }
